...
public struct DataURL: Codable {
    ...
    static let dataUrlRegex = try! NSRegularExpression(pattern: #"^data:(.+?)?(;base64)?,(.+)$"#, options: [])
    
    var contentPart: String? {
        if base64 {
            return data.base64EncodedString()
        } else {
            return String(data: data, encoding: .utf8)?.addingPercentEncoding(withAllowedCharacters: .urlPasswordAllowed)
        }
    }
